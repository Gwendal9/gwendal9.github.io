...
const DOCKER_LOGO = 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg'
...
function WorkflowSVG({ uid='a' }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{display:'block',overflow:'visible'}}>
      ...
      <text x={T1X+42} y={CY1-4} fontSize="11" fontWeight="700" fill='var(--ink)' fontFamily="system-ui,sans-serif">TenUp</text>
      <text x={T1X+42} y={CY1+10} fontSize="8.5" fill='var(--low)' fontFamily="monospace">API non-doc</text>
      ...
      <text x={P1R-12} y={P1Y+24} fontSize="8.5" fill='var(--low)' fontFamily="monospace" textAnchor="end">15 workers async</text>
      <line x1={P1X+1} y1={P1Y+38} x2={P1R-1} y2={P1Y+38} stroke='var(--border)' strokeWidth="1"/>
      ...
      {QUEUE_STATES.map((state,i)=>{
        ...
        return (
          <g key={state}>
            ...
            <text x={cx} y={cy+4} textAnchor="middle" fontSize="10" fontWeight="700" fill='var(--ink)' fontFamily="monospace">{state}</text>
          </g>
        )
      })}
      ...
      <text x={DB1CX} y={DB1Y+52} textAnchor="middle" fontSize="11" fontWeight="700" fill='var(--ink)' fontFamily="system-ui,sans-serif">PostgreSQL</text>
      <text x={DB1CX} y={DB1Y+67} textAnchor="middle" fontSize="8.5" fill='var(--low)' fontFamily="monospace">156k joueurs</text>

      <text x={W-10} y={CY1+4} textAnchor="end" fontSize="9" fill={GR} fontFamily="monospace" fillOpacity="0.55">scraping pipeline</text>

      <line x1={8} y1={(CY1+CY2)/2} x2={W-8} y2={(CY1+CY2)/2} stroke='var(--border)' strokeWidth="1" strokeDasharray="4 8"/>
      ...
      <text x={C2X+C2W/2} y={CY2+13} textAnchor="middle" fontSize="8.5" fill='var(--low)' fontFamily="monospace">1er mardi/mois</text>
      ...
      <text x={F2CX} y={F2Y+50} textAnchor="middle" fontSize="11" fontWeight="700" fill='var(--ink)' fontFamily="system-ui,sans-serif">Flask API</text>
      <text x={F2CX} y={F2Y+64} textAnchor="middle" fontSize="8.5" fill='var(--low)' fontFamily="monospace">REST · H/F</text>

      {/* nginx */}
      <rect x={N2X} y={N2Y} width={N2W} height={N2H} rx={9} fill="var(--white)" stroke={VIB} strokeWidth="1.2"/>
      <text x={N2CX} y={CY2-1} textAnchor="middle" fontSize="11" fontWeight="700" fill='var(--ink)' fontFamily="monospace">nginx</text>
      <text x={N2CX} y={CY2+14} textAnchor="middle" fontSize="8.5" fill='var(--low)' fontFamily="monospace">reverse proxy</text>
      ...
      <text x={D2CX} y={D2Y+48} textAnchor="middle" fontSize="11" fontWeight="700" fill='var(--ink)' fontFamily="system-ui,sans-serif">Dashboard</text>
      <text x={D2CX} y={D2Y+63} textAnchor="middle" fontSize="8.5" fill='var(--low)' fontFamily="monospace">HTML/JS vanilla</text>
      ...
      <text x={DK2X+38} y={DK2Y+40} fontSize="8.5" fill='var(--low)' fontFamily="monospace">nginx · Render</text>
